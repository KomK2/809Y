...
void navigation::NavigationDemo::send_goal() {
  ...
  RCLCPP_INFO(this->get_logger(), "subs Colors , x is: %f,", pose_array_data.poses[0].position.x);
       RCLCPP_INFO(this->get_logger(), "subs Colors , y is: %f,", pose_array_data.poses[0].position.y);
         RCLCPP_INFO(this->get_logger(), "subs Colors , z is: %f,", pose_array_data.poses[0].position.z);
       RCLCPP_INFO(this->get_logger(), "subs Colors , ori x is: %f,", pose_array_data.poses[0].orientation.x);
       RCLCPP_INFO(this->get_logger(), "subs Colors , ori y is: %f,", pose_array_data.poses[0].orientation.y);
       RCLCPP_INFO(this->get_logger(), "subs Colors , ori z is: %f,", pose_array_data.poses[0].orientation.z);
       RCLCPP_INFO(this->get_logger(), "subs Colors , ori w is: %f,", pose_array_data.poses[0].orientation.w);
  ...
  // goal_msg.poses[0].pose.position.x = 1.918330 ;
  // goal_msg.poses[0].pose.position.y = -2.504200;
  // goal_msg.poses[0].pose.position.z = 0.0;
  // goal_msg.poses[0].pose.orientation.x = 0.0;
  // goal_msg.poses[0].pose.orientation.y = 0.0;
  // goal_msg.poses[0].pose.orientation.z = 0.706825;
  // goal_msg.poses[0].pose.orientation.w = 0.70738;
...
void  navigation::NavigationDemo::poseArrayCallback(const geometry_msgs::msg::PoseArray::SharedPtr msg) {
